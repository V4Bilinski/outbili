...
### F3B — Re-arquitetura do worker (cadastral) `[✅ em produção]`
...
- [x] **Infra aplicada em produção (2026-05-30, direto em prod + smoke):**
  - [x] Migration via MCP `apply_migration` → `{"success":true}`.
  - [x] Edge `assertiva-enrich` via CLI `supabase functions deploy` (projeto `yxppliytwvlajeqqrrny`).
  - [x] **Smoke pegou e corrigiu um bug:** Assertiva devolve data DD/MM/YYYY → gravar cru na coluna `date` corrompia `founding_date` (12/06 → 06/dez). Fix: conversão p/ ISO na Edge (ignora formato desconhecido). Re-deploy + re-teste OK. Lead de teste restaurado.
  - [x] **Caminho da fila validado:** job `metadata.mode='cadastral'` → `dispatch` roteou → Edge gravou `years_in_market`+`founding_date` (ISO) → job `done`.
  - [x] Frontend mergeado na `main` (`17014d1`) + deploy Actions SUCCESS + bundle de produção validado.
